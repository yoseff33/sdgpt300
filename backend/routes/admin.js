...
const audit = async (req, action, targetType, targetId, details = {}) => {
  const { error } = await admin.from('admin_audit_logs').insert({
    admin_id: req.user.id, action, target_type: targetType, target_id: String(targetId || ''),
...
router.get('/overview', async (_req, res) => {
  ...
  res.json({
    users: userRows.length, active_users: userRows.filter(x => x.account_status === 'active').length,
    ...
    products: productRows.length, suspended_products: productRows.filter(x => ['suspended','rejected'].includes(x.moderation_status)).length,
    transactions: tx.length, open_disputes: (disputes.data || []).filter(x => x.status === 'open').length,
...
router.get('/users', async (req, res) => {
  ...
  if (req.query.q) query = query.or(`full_name.ilike.%${String(req.query.q).slice(0,80)}%,phone.ilike.%${String(req.query.q).slice(0,80)}%`);
...
router.get('/users/:id', async (req, res) => {
  ...
  res.json({ profile: profile.data, purchases: purchases.data || [], sales: sales.data || [], products: products.data || [], alerts: (alerts.data || []).filter(x => transactionIds.includes(x.transaction_id)) });
});
